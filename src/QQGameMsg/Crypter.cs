...
namespace QQGameMsg
{
    public class Crypter
    {
        ...
        ///     int使用uint表示的。如果len大于4，则认为len等于4。如果len小于4，则高位填0
        ///     <br>
        ...
        {
            if (key == null) throwException(@"key is null.");
            // 迭代次数，16次
            var loop = 0x10;
            ...
            var y = GetUInt(input, 0, 4);
            var z = GetUInt(input, 4, 4);
            var a = GetUInt(key, 0, 4);
            var b = GetUInt(key, 4, 4);
            var c = GetUInt(key, 8, 4);
            var d = GetUInt(key, 12, 4);
        ...
        private byte[] Decipher(byte[] input, int offset)
        {
            ...
            // 我们用了uint，这个uint的前32位是全0的，我们通过这种方式模拟无符号整数，后面用到的uint也都是一样的
            // 而且为了保证前32位为0，需要和0xFFFFFFFF做一下位与
            var y = GetUInt(input, offset, 4);
            var z = GetUInt(input, offset + 4, 4);
            var a = GetUInt(key, 0, 4);
            var b = GetUInt(key, 4, 4);
            var c = GetUInt(key, 8, 4);
            var d = GetUInt(key, 12, 4);
